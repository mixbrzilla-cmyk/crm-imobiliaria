  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white">
        ...
        {capturedError ? <div className="p-5 text-sm text-red-700">{capturedError}</div> : null}

        {capturedLoading ? (
          <div className="p-5 text-sm text-zinc-600">Carregando...</div>
        ) : capturedRows.length === 0 ? (
          <div className="p-5 text-sm text-zinc-600">Você ainda não captou imóveis.</div>
        ) : (
              ...
              return (
                <div key={`${r.source}:${r.id}`} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-semibold text-zinc-900">{title || "Sem título"}</div>
                    ...
                    <div className="mt-2 grid grid-cols-1 gap-1 text-xs text-zinc-700 sm:grid-cols-2">
                      ...
                      {ownerWhatsapp ? <div className="sm:col-span-2">WhatsApp do proprietário: {ownerWhatsapp}</div> : null}
                    </div>
                  </div>
